function figure15(decodeMatrix, opt)
...
set(f,'UserData', {'figure15', {decodeMatrix, opt}});

subplot('Position', [0.08 0.25 0.88 0.7]); hold on;
imagesc(decodeMatrix, 'ButtonDownFcn', @graphChange);
plot([0; opt.nStim*opt.nVols], repmat((opt.nVols+1):opt.nVols:(opt.nVols*opt.nVox),2,1), ':', 'Color', [0.5 0.5 0.5]);
plot(repmat((opt.nStim+0.5):opt.nStim:(opt.nStim*opt.nVols),2,1), [0; opt.nVox*opt.nVols], ':', 'Color', [0.5 0.5 0.5]);
